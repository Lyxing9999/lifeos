...
public interface ScheduleBlockJpaRepository extends JpaRepository<ScheduleBlock, UUID> {
    List<ScheduleBlock> findByUserIdAndArchivedFalseOrderByStartTimeAsc(UUID userId);
    List<ScheduleBlock> findByUserIdAndArchivedTrueOrderByStartTimeAsc(UUID userId);
}