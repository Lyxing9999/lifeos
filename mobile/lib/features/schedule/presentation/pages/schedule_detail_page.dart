    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          if (block.active)
            IconButton(
              tooltip: ScheduleCopy.deactivateTooltip,
              onPressed: () async {
                final confirmed = await showDialog<bool>(
                  context: context,
                  builder: (ctx) => AlertDialog(
                    title: const Text(ScheduleCopy.deactivateDialogTitle),
                    content: Text(
                      ScheduleCopy.deactivateDialogBody(block.title),
                    ),
                    actions: [
                      TextButton(
                        onPressed: () => Navigator.of(ctx).pop(false),
                        child: const Text(ScheduleCopy.cancelAction),
                      ),
                      FilledButton(
                        onPressed: () => Navigator.of(ctx).pop(true),
                        child: const Text(ScheduleCopy.deactivateDialogAction),
                      ),
                    ],
                  ),
                );
                ...
                await ref
                    ...
                    .deactivate(userId: userId, id: block.id);
          ...
          IconButton(
            onPressed: () async {
                    ...
                    builder: (context, ref, child) {
                      ...
                      return ScheduleFormPage(
                        ...
                        onSubmit: (result) async {
                          ...
                          Navigator.of(
                            context,
                          ).pop(ScheduleCopy.successUpdated);
                        },
                      );
                    },
                  ),
              ...
              await ref
                  .read(scheduleNotifierProvider.notifier)
                  .loadById(userId: userId, id: id);
            },
            icon: const Icon(Icons.edit_outlined),
          ),
          IconButton(
            tooltip: ScheduleCopy.deleteTooltip,
            onPressed: () async {
              final confirmed = await showDialog<bool>(
                context: context,
                builder: (ctx) => AlertDialog(
                  title: const Text(ScheduleCopy.deleteDialogTitle),
                  content: Text(ScheduleCopy.deleteDialogBody(block.title)),
                  actions: [
                    TextButton(
                      onPressed: () => Navigator.of(ctx).pop(false),
                      child: const Text(ScheduleCopy.cancelAction),
                    ...
                    FilledButton(
                      ...
                      onPressed: () => Navigator.of(ctx).pop(true),
                      child: const Text(ScheduleCopy.deleteDialogAction),
                    ),
                  ],
                ),
              );
              ...
              await ref
                  .read(scheduleNotifierProvider.notifier)
                  .delete(userId: userId, id: block.id);
              if (context.mounted) Navigator.of(context).pop();
            },
