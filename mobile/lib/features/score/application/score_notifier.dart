...
class ScoreNotifier extends StateNotifier<ScoreState> {
  ...
  Future<void> load({required DateTime date}) async {
    ...
    try {
      final score = await repository.getDailyScore(date: date);
  ...
  Future<void> generate({required DateTime date}) async {
    ...
    try {
      final score = await repository.generateDailyScore(date: date);
  ...
  Future<void> delete({required DateTime date}) async {
    ...
    try {
      await repository.deleteDailyScore(date: date);
